...
public class sphere {
    ...
    // Define resolution for generating points (higher values give smoother surfaces)
...
ellipsoidPoints[ellipsoidPoints.length - 1] = new Vec3d(
    ...
    // Filter and remove duplicates based on seenPoints
    Vec3d[] uniquePoints = {};
...
seenPoints[seenPoints.length - 1] = point;
                ...
                for (Vec3d element : uniquePoints) {
                    networkHandler.sendChatCommand("setblock " + (long) element.x + " " + (long) element.y + " " + (long) element.z + " " + blockStateString);
                }
